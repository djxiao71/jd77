...
Name1=(fruit pet plantBean dreamFactory jdfactory crazy_joy jdzz jxnc bookshop cash sgmh cfd global carnivalcity)
Name2=(东东农场 东东萌宠 京东种豆得豆 京喜工厂 东东工厂 crazyJoy任务 京东赚赚 京喜农场 口袋书店 签到领现金 闪购盲盒 京喜财富岛 环球挑战赛 京东手机狂欢城)
Name3=(Fruit Pet Bean DreamFactory JdFactory Joy Jdzz jxnc BookShop Cash Sgmh Cfd Global JD818)
...
function Cat_Scodes {
  if [ -d ${LogDir}/jd_$1 ] && [[ $(ls ${LogDir}/jd_$1) != "" ]]; then
    ...
    for log in $(ls -r); do
      case $# in
        1)
          codes=$(cat ${log} | grep -${Opt} "开始【账号|您的(好友)?助力码为" | uniq | perl -0777 -pe "{s|\*||g; s|开始||g; s|\n您的(好友)?助力码为(：\|:\| )?|：|g; s|，.+||g}" | perl -ne '{print if /：/}')
          ;;
        2)
...
Cat_All | perl -pe "{s|种豆|种豆|; s|crazyJoy任务|疯狂的JOY|}" | tee ${LogFile}
